...
def download_and_load_data():
    if not os.path.exists("Police_Incident_Reports.csv"):
        file_id = '1-DsZwyEd0dmHEdgnCtO4NvumNHmnV567'
        url = f'https://drive.google.com/uc?id={file_id}'
        gdown.download(url, 'Police_Incident_Reports.csv', quiet=False)

    df = pd.read_csv('Police_Incident_Reports.csv', low_memory=False)
...
def load_data():
    df = pd.read_csv("Police_Incident_Reports__2018_to_Present_20250622.csv", low_memory=False)
    df["Incident Datetime"] = pd.to_datetime(df["Incident Datetime"], errors="coerce", utc=True)
    df = df.dropna(subset=["Incident Datetime", "Latitude", "Longitude"])
    df["date"] = df["Incident Datetime"].dt.floor("D")
    df["hour"] = df["Incident Datetime"].dt.hour
